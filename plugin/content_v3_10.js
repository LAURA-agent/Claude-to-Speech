...
async processStreamUpdate() {
  ...
  try {
    ...
    console.log("DEBUG - textToProcess:", textToProcess.substring(0, 50));
console.log("DEBUG - lastCleanedText:", this.lastCleanedText?.substring(0, 50));
    ...
    if (this.lastCleanedText && textToProcess === this.lastCleanedText) {
      ...
      if (this.currentResponseText.length > 0) {
        ...
        newText = textToProcess;
        console.log("DEBUG - newText calculation:");
        console.log("  newText:", newText.substring(0, 50));
        console.log("  newText.length:", newText.length);
        console.log("  isStreaming:", this.isClaudeTyping(responseElement));
      }
    }

    if (newText.length > 0) {
      if (this.isClaudeTyping(responseElement)) {
        const paragraphBreak = newText.indexOf('\n\n');
        
        if (paragraphBreak !== -1) {
          const textToSend = newText.substring(0, paragraphBreak).trim();
          if (textToSend.length > 20) {
            const baseId = this.generateResponseId(this.currentResponseElement);
            const responseId = `${baseId}-para-${this.chunkCounterForElement++}`;
            await this.sendStreamChunk(textToSend, false, responseId);
          ...
          if (sentenceMatch.found && sentenceMatch.text.length > 50) {
            const baseId = this.generateResponseId(this.currentResponseElement);
            const responseId = `${baseId}-sent-${this.chunkCounterForElement++}`;
            await this.sendStreamChunk(sentenceMatch.text, false, responseId);
            this.lastCleanedText = textToProcess;
            this.processedChunks.set(this.simpleHash(sentenceMatch.text), true);
          }
        }
      } else {
        const baseId = this.generateResponseId(this.currentResponseElement);
  ...
  isClaudeTyping(responseElement) {
    console.log("DEBUG - isClaudeTyping checks:", responseElement?.getAttribute('data-is-streaming'));
    console.log("DEBUG - closest streaming:", responseElement.closest('[data-is-streaming="true"]'));
    console.log("DEBUG - send button disabled:", document.querySelector('button[type="submit"]:disabled'));
    ...
    for (const selector of typingSelectors) {
      console.log("DEBUG - checking selector:", selector, responseElement.querySelector(selector));
